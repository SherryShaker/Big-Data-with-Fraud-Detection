...
spark = SparkSession.builder.config("spark.driver.memory", "20g").appName("Train_ml").getOrCreate()
train = spark.read.options(header='true').csv("100k.csv")
...
# get updated categorical
categorical = list(set(categorical) - set(to_drop))
# get updated numerical
...
layers = [50,7,2] # 50 the number of total features, 2 stands for isFraud or not
mlp = MultilayerPerceptronClassifier(featuresCol="features", labelCol="isFraud", layers=layers, blockSize=128, seed=1234)
...
grid = ParamGridBuilder().addGrid(mlp.maxIter, [20]).build()
